...
public class Texture : IDisposable, IBindable
{
	...
	public void Allocate(int width, int height, SizedInternalFormat internalFormat, int levels = -1)
	{
		if (levels < 0)
		{
			levels = BitOperations.Log2((uint)Math.Max(width, height));
		}
	...
	public void Update(nint data, int x, int y, int width, int height, PixelFormat format, PixelType pixelType, int level = 0)
	{
	...
	public void ReadData(nint data, int bufferSize, PixelFormat format, PixelType type, int level = 0)
	{
